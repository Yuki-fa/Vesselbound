...
function waveLifeMax(){
  // 生命の指輪：常時：ライフ上限が+1される。宿屋で上限まで回復できる。
  const ringBonus=typeof _ringCount==='function'?Math.max(0,Number(_ringCount('生命の指輪'))||0):0;
  if(typeof G!=='undefined'&&G&&G._onlineMode){
    return ((typeof ONLINE_START_LIFE!=='undefined'?Number(ONLINE_START_LIFE):5)||5)+ringBonus;
  }
  return 3+ringBonus;
}
...
// キーワードの説明文。**シートの「キーワード」シートが正**で、loader.js が上書きする。
// ここはシートを読めない時の予備。
// **強化カード名（攻防一体・熟練・大盾など）はここに書かない。**
// あれらはカードそのものなので、説明は「エンチャント」シートの効果列が正
// （引き方は render.js の _enchantKeywordDesc）。
const KW_DESC_MAP={
  ...
  '封印':     'このキャラクターは攻撃できず、いかなる効果も受けない。血がX以上になると解放される。',
  ...
  '隠密':     'このキャラクターは「隠密」を持たないキャラクターがいる限り、敵の攻撃の対象にならない。',
...
// **シートのNo.を振り直したらここも直すこと。** 古い番号のままだと、別のキーワードの
// 素材を拾う（毒がK007のままだった時、毒ダメージで貫通のSEが鳴り、VFXは出なかった）。
const KW_NO_MAP={
  '即死': 'K001',
  '毒牙': 'K003',
  '衝撃': 'K004',
