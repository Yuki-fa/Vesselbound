    ...
    if(!options?.resume&&!(G&&G._onlineMode)&&(type==='elite'||type==='boss'||type==='finalBoss')&&typeof _ensureWaveEnemyPreview==='function'){
      ...
      if(enemyPreview&&enemyPreview.def){
        ...
        const payload={
          ...
          name:def.name,desc:(typeof _annotateSummonNames==='function'?_annotateSummonNames(String(def.desc||'').trim(),{...def,atk:enemyPreview.atk,maxHp:enemyPreview.hp}):String(def.desc||'').trim()),atk:enemyPreview.atk,hp:enemyPreview.hp,art:artPaths||null,
