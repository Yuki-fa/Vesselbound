...
async function loadGameData() {
  try {
    ...
    // 召喚でしか出ないカード（報酬・ショップに出さない）。
    // **C022＝シャドウ／C101＝スケルトンへ入れ替え。** 召喚されるのは「青スケルトン」で、
    // 「青シャドウ」を召喚する効果は無くなったため、summon専用はスケルトン側になる。
    const _summonOnlyPanelNames = new Set(['スケルトン', 'ウルフ', 'ペリカン', 'ドラゴン', 'ナイトキャット', 'イフリート'].map(_normCardName));
    ...
    const _setPanelKeywordsFromDesc = panel => {
      ...
      const colorBuffAll = passiveDesc.match(/死亡：全ての([赤青緑黄紫茶])(?:の味方|の?キャラクター)は\+(\d+)\/\+(\d+)を得る/);
    ...
    const _syncPanelFromRow = (panel, row, forcedCategory) => {
      ...
      // 「VFX/SE」列＝そのカードの効果で使う演出の番号（No.とは別に指定できる）。
      // 空欄ならカード自身のNo.を使う。キーワードのVFX/SEはここで指定しなくても出る。
      const fxCodeVal = String(row['VFX/SE'] ?? row['VFX'] ?? '').trim().toUpperCase();
      if (fxCodeVal) panel.fxCode = fxCodeVal;
      else delete panel.fxCode;
      // 「マナ順位」列＝マナ効果の処理順。小さいほど先に発動する。
      // 空欄は順位なし（最後に回す）。キャラクター・エンチャントの両シート共通。
      const manaOrderVal = parseInt(String(row['マナ順位'] ?? '').trim(), 10);
      if (!isNaN(manaOrderVal)) panel.manaOrder = manaOrderVal;
      else delete panel.manaOrder;
    ...
    // **シートの「キーワード」「効果」列と完全に同じ値にする。** 効果欄が空のカードは desc も空（シート読込と同じ。
    // キーワードを説明文へ写すと同じ語が2つ並ぶ）。シートを直したらここも合わせること。
    const _requestedEffectOverrides = {
      'ノーム': {desc:'終戦：20ゴールドを得る。'},
      'ゴーレム': {desc:'負傷：このキャラクターは+2/+2を得る。', keywords:['根性']},
      'ドワーフ': {desc:'2マナ毎：ランダムな赤の味方2体は+3/+2を得る。'},
      ...
      'スリン': {desc:'', keywords:['毒牙3','邪眼3']},
      'ミテーラ': {desc:'開戦：「緑ペリカン」を2体召喚する。'},
      'ユミル': {desc:'攻撃：このキャラクターは+X/+Xを得る。Xはマナに等しい。'},
      'マーメイド': {desc:'常時：緑の味方から得るマナは+1される。'},
      'グリムリーパー': {desc:'', keywords:['封印5','即死']},
      'ハイドラ': {desc:'終戦：このキャラクター以外の、生存したキャラクターが報酬に1枚出現する。'},
      'コカトリス': {desc:'4マナ毎：ランダムな敵に防戦を与える。'},
      'スキュラ': {desc:'2マナ毎：全ての敵に毒1を与える。'},
      'ナーガ': {desc:'常時：戦闘中に召喚された味方は+1/+1を得る。戦闘中に召喚された味方の数だけ繰り返す。'},
      'ブラウニー': {desc:'攻撃＆負傷：全ての仲間のHPが+2される。'},
      'エルヴンメイジ': {desc:'攻撃：全ての黄の味方は+1/+1を得る。'},
