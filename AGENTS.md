...
- 【今回追加】魔導板初期配置・鍛冶屋抽選・村/ボス後置き仕様を実装（reward.js／battle.js／main.js／map.js／index.html）。未コミット。
  - 魔導板の初期召喚枠を `[1,3,10,12,14]`（■□■□■／■■■■■／□■□■□）へ変更し、通常枠と召喚/特殊枠を `mapPanelPowerIdAt()` で共通判定するよう整理。戦闘開始時も見た目と同じ位置で出撃するよう補正。
  - 鍛冶屋は項目選択だけで抽選まで実行する方式へ変更。「召喚の力」は最上段通常枠から、その他は既存召喚枠からランダム選択し、カードをフェードアウトしつつ候補スロットを巡回発光してから変化する。
  - 村はマップ生成時に配置せず、2ターン後にプレイヤーから2歩の未知マスへ初回配置し、その後同マップの残り村を確定。ボスは生成時に決めるが、10ターン目まで未発見ならプレイヤーから3歩の未知マスへ再配置。開始地点隣接マスは全マップで通常戦闘固定。
  - `node --check js/engine/map.js`, `reward.js`, `battle.js`, `main.js`通過。Node簡易試行120回で開始隣接通常戦闘固定、2ターン村配置、10ターン未発見ボス再配置を確認。実機目視は未実施。
- 【今回追加】計算式/ルール確認に基づくズレを修正（map.js／battle.js／reward.js／pool.js）。未コミット。
  - ターン補正を `1+現在ターン*0.05` に変更し、敵強さ星評価の基準も同じ係数に合わせた。
  - 初回敵数制限を「1マップ目・通常戦闘の1戦目は1体/2戦目は2体・4ターン以内」に限定。通常戦闘専用カウントを追加し、エリート/ボスに適用されないようにした。
  - マップ施設制約の「隣接」「3マス未満」をグリッド座標ではなく道のグラフ距離で判定するよう変更。未知エリート出現も初期地点から4歩以上に限定。
  - 黄金の巻物は即時所持金2倍に変更し、古い「次戦で敵強化/敵ゴールド2倍」処理を削除。`node --check`はmap/battle/reward/pool通過。
