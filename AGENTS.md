...
- サキュバス（奪う）・レムレース（死亡したキャラを召喚）：奪った体・敵から召喚した体は両経路とも `enemy_frame`（緑にならない）。
- ゴースト：死亡のバフはシートの VFX/SE（S007「バフ（死亡）」）を使う経路。検査の体は fxCode を持たないため、実際の絵は未確認。
- 結界VFXの残り：board.js の shield_lost で盾・結界バッジを消す修正が既にある（コード確認のみ）。
- 待機中のボタン反応：`body.online-waiting` で街のボタンの入力を止める CSS が既にある（コード確認のみ）。
...
- 召喚上限：前衛7体で満員の時、スケルトンキングの召喚は `summon_rejected`（7/14）になり、画面のカードも7枚のまま（両経路）。空き1なら両経路で召喚される。
- **盤面の詰め（FLIP）**：倒れた時に残りのカードが滑らず1フレームで動く不具合があった（PvE は全部、オンラインは敵側）。**2026-09-15 解消（ヘッドレス）。**
