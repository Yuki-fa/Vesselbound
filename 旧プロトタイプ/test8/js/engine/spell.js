...
function _selectedUnitCanAct(unit){
  ...
  if(typeof isManualActiveUnit==='function'&&G._manualTurnActive) return isManualActiveUnit(unit)&&!unit._actedThisTurn;
...
function useSpell(idx){
  ...
  if(G.phase==='player'&&owner&&sp.type==='weapon'){
    G._previewWeaponPower={unitId:owner.id,power:sp.power==='durability'?Math.max(0,sp.usesLeft||0):(Number(sp.power||0)||0)};
    if(typeof renderField==='function') renderField('f-ally',G.allies,false);
...
function pickWeaponTarget(idx){
  ...
  const previewTargets=i=>_weaponTargetRefs(sp,i,owner).filter(ref=>ref.side==='enemy').map(ref=>ref.i);
  _getEnemyDomSlots().forEach((slot,i)=>{
    const e=G.enemies[i];
    ...
    if((e&&e.hp>0)||allowEmpty){
      slot.classList.add('selectable');
      slot.onmouseenter=()=>{
        _getEnemyDomSlots().forEach(s=>s.classList.remove('glow-red'));
        previewTargets(i).forEach(ti=>_getEnemyDomSlots()[ti]?.classList.add('glow-red'));
      };
      slot.onmouseleave=()=>_getEnemyDomSlots().forEach(s=>s.classList.remove('glow-red'));
      slot.onclick=()=>{ clearSelectable(); applyWeapon(sp,idx,i); };
    }
  });
  if(sp.weaponMode==='all'||sp.weaponMode==='random_any'){
    [..._getEnemyDomSlots(),..._getAllyDomSlots()].forEach(slot=>slot.classList.add('glow-red'));
  }
  _addCancelListeners();
}
...
function _weaponTargetRefs(sp,targetIdx,attacker){
  ...
  if(mode==='random_any'){
    const refs=[];
    G.enemies.forEach((u,i)=>{ if(u&&u.hp>0) refs.push({side:'enemy',u,i,w:80}); });
    G.allies.forEach((u,i)=>{ if(u&&u.hp>0) refs.push({side:'ally',u,i,w:20}); });
    if(!refs.length) return [];
    const total=refs.reduce((s,r)=>s+r.w,0);
    let roll=Math.random()*total;
    for(const ref of refs){ roll-=ref.w; if(roll<=0) return [ref]; }
    return [refs[0]];
  }
...
function _damageWeaponTarget(ref,dmg,attacker,sp){
  ...
  if(sp.weaponKeyword==='lower_atk_dealt'&&ref.u&&ref.u.hp>0){
    ref.u.atk=Math.max(0,(ref.u.atk||0)-Math.max(0,before-ref.u.hp));
  }
  if(sp.weaponKeyword==='poison20'&&ref.u&&ref.u.hp>0){
    ref.u.poison=Math.max(ref.u.poison||0,Math.ceil((ref.u.maxHp||ref.u.hp)*0.2));
  }
...
function applyWeapon(sp,idx,targetIdx){
  ...
  if(sp.weaponKeyword==='party_atk1'&&owner.hp>0){
    G.allies.forEach(a=>{ if(a&&a.hp>0){ a.atk+=1; a.baseAtk=(a.baseAtk||0)+1; } });
    log(`${sp.name}：仲間全体のパワー+1`,'good');
  }
  applyRingAttackAfterDamage(owner,totalDealt,killedEnemy);
...
function clearSelectable(){
  ...
  document.removeEventListener('keydown',_cancelSpreadKD);
  document.removeEventListener('contextmenu',_cancelSpreadCM);
  document.querySelectorAll('.selectable').forEach(s=>{ s.classList.remove('selectable'); s.onclick=null; s.onmouseenter=null; s.onmouseleave=null; });
  document.querySelectorAll('.glow-red').forEach(s=>s.classList.remove('glow-red'));
  document.querySelectorAll('.bless-blocked').forEach(s=>s.classList.remove('bless-blocked'));
  if(G._previewWeaponPower){ G._previewWeaponPower=null; if(typeof renderField==='function') renderField('f-ally',G.allies,false); }
}
