...
async function advanceManualTurn(){
  if(G.phase!=='player') return;
  const queue=G._manualTurnQueue||[];
  while(G._manualTurnPos<queue.length){
    const ref=queue[G._manualTurnPos++];
    const current=ref.side==='enemy'?G.enemies[ref.idx]:G.allies[ref.idx];
    if(!current||current.id!==ref.unit.id||current.hp<=0||current._isObject||current._isSoul) continue;
    G._manualTurnActive={side:ref.side,idx:ref.idx,unitId:current.id};
    if(ref.side==='ally'){
      selectAllyLoadout(ref.idx);
      syncManualActionCounts();
      renderAll();
      setHint(`${current.name}の行動を選択してください。`);
      return;
    }
    current._actedThisTurn=true;
    clearAllyLoadoutSelection?.();
    log(`── ${current.name}のターン ──`,'bad');
    renderAll();
    await sleep(250);
    await enemyAttackAction(current,ref.idx);
...
function startPlayerPhase(){
  ...
  setHint(liveA.length===0?'仲間がいない！':'行動順にターンが回ります。');
  if(typeof advanceManualTurn==='function') advanceManualTurn();
}
...
function _onAllEnemiesDefeated(){
  ...
  if(typeof grantExpToSurvivingAllies==='function') grantExpToSurvivingAllies(G._pendingBattleExp||0);
  G._pendingBattleExp=0;
  applyVictoryBonuses();
  updateHUD(); renderAll();
  G.phase='reward';
...
function _enemyActionUsable(action){
  const item=action&&action.item;
  return !!item&&(item.usesLeft===undefined||item.usesLeft>0);
}
function _consumeEnemyAction(action, enemy){
  const item=action&&action.item;
  if(!item||item.noConsume) return;
  if(item.usesLeft!==undefined) item.usesLeft=Math.max(0,item.usesLeft-1);
  if(item.usesLeft===0) log(`${enemy.name}の${item.name}が壊れた`,'sys');
}
function _pickEnemyAction(enemy){
  const actions=(enemy.enemyActions||[]).filter(_enemyActionUsable);
  if(!actions.length) return null;
  const heal=actions.find(a=>a.item&&a.item.effect==='enemy_heal_wounded');
  if(heal){
    const targets=G.enemies.filter(e=>e&&e.hp>0&&!e._occupiedEnemy&&(e.maxHp||e.hp)-e.hp>=15);
    if(targets.length&&Math.random()<(heal.prob||0)) return heal;
  }
  const attacks=actions.filter(a=>a.item.effect!=='enemy_heal_wounded');
  const total=attacks.reduce((s,a)=>s+(a.prob||1),0)||1;
...
async function enemyAttackAction(enemy, enemyIdx){
  ...
  const action=_pickEnemyAction(enemy);
  const item=action&&action.item;
  if(item&&item.effect==='enemy_heal_wounded'){
    const targets=G.enemies.filter(e=>e&&e.hp>0&&!e._occupiedEnemy&&(e.maxHp||e.hp)-e.hp>=15);
    const target=targets.length?targets.sort((a,b)=>((b.maxHp||b.hp)-b.hp)-((a.maxHp||a.hp)-a.hp))[0]:null;
    if(target){
      const heal=Math.max(1,Number(item.power)||5);
      target.hp=Math.min(target.maxHp||target.hp,target.hp+heal);
      log(`${enemy.name}：${item.name}→${target.name}を${heal}回復`,'bad');
      _consumeEnemyAction(action,enemy);
      renderAll();
      await sleep(300);
      return;
    }
  }
  if(item&&item.effect==='enemy_ancient_power'){
    enemy._enemyNextPowerMult=2;
    log(`${enemy.name}：${item.name}→次の攻撃を強化`,'bad');
    _consumeEnemyAction(action,enemy);
    renderAll();
    await sleep(300);
    return;
  ...
  const equipPower=item?Number(item.power)||0:0;
  const powerMult=enemy._enemyNextPowerMult||1;
  const atkVal=enemy.nullified>0?0:Math.max(0,(enemy.atk+equipPower)*powerMult);
  enemy._enemyNextPowerMult=1;
  ...
  log(`${enemy.name}${item?`：${item.name}`:''}(${atkVal})→${isGlobalAtk?'全体':isTriDirAtk?`${primaryTarget.name}周辺3体`:hitNames.join('・')}`);
  finalTargets.forEach(tgt=>{
    ...
    if(!hitSet.has(tgt.id)){
      ...
      if(item&&item.effect==='enemy_poison_attack'&&actualTarget&&actualTarget.hp>0){
        actualTarget.poison=(actualTarget.poison||0)+3;
        log(`${item.name}：${actualTarget.name}に毒+3`,'bad');
      }
  ...
  if(item) _consumeEnemyAction(action,enemy);
...
function processEnemyDeath(e,eIdx){
  ...
  if(!G._isSimulating){
    const gainedExp=Math.max(1,e.exp||Math.round((e.maxHp||1)/5));
    G._pendingBattleExp=(G._pendingBattleExp||0)+gainedExp;
    log(`${e.name}：経験値+${gainedExp}`,'sys');
  }
