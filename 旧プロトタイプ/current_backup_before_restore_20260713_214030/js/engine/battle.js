...
async function startBattle(){
  ...
  G.mana=0;
  G.allies.forEach(a=>{ if(a){ delete a._deathProcessed; delete a._manaThresholdFired; } });
  G.enemies.forEach(e=>{ if(e){ delete e._deathProcessed; delete e._manaThresholdFired; } });
...
async function _applyAllyAttackEffects(ally){
  ...
  if(ally&&/常時：このキャラクターの攻撃効果は1回追加で発動する。/.test(String(ally.desc||''))&&ally.hp>0){
    await _applyUnitAttackEffects(ally,false);
  }
}
...
async function _applyEnemyAttackEffects(enemy){
  ...
  if(enemy&&/常時：このキャラクターの攻撃効果は1回追加で発動する。/.test(String(enemy.desc||''))&&enemy.hp>0){
    await _applyUnitAttackEffects(enemy,true);
  }
}
...
function _applyDamageState(unit, dmg, source, side){
  ...
  // 弱体化X：このキャラクターが受ける1以上のダメージはX増加する
  const weakenKw=(unit.keywords||[]).find(k=>/^弱体化\d+$/.test(k));
  if(weakenKw) dmg+=parseInt(weakenKw.slice(3),10)||0;
  ...
  if(actualDmg>0&&unit.hp>0){
    if(side==='ally'){
      ...
      const fired=_fireAllyInjuryEffects(unit,actualDmg);
      if(fired){
        _bumpEtinOnAllyInjuryEffect();
        const leftN=_leftNeighborAlly(unit);
        if(leftN&&leftN.hp>0&&leftN.name==='フォルモール'){
          _fireAllyInjuryEffects(unit,actualDmg);
          _bumpEtinOnAllyInjuryEffect();
        }
      }
    } else {
...
function _syncUnitBattleOrder(unit){
  if(!unit) return [];
  const eq=Array.isArray(unit.equipment)?unit.equipment:[];
  const currentPanels=eq.filter(panel=>panel&&String(panel.category||'')==='キャラクター');
...
// ── 「リーダー」＝プレイヤー陣営の後衛（後列）にいる唯一のキャラクター ──
...
// 色が付いた名前（例：「赤ゴーレム」）は色部分を色分類に、残りを名前として扱う。
// 永続色強化（G.panelColorPermanentBuffs）が積まれていればその色分だけ加算する。
function _spawnAdhocAllyUnit(name, atk, hp, isEnemySide){
  // 表示名は「青スケルトン」のように色接頭辞込みのまま保持し、色は永続色強化の参照用にのみ別途取り出す
  const m=String(name||'').match(/^([赤青緑黄紫])(.+)$/);
  const color=m?m[1]:'';
  const fullName=String(name||'');
  let finalAtk=Number(atk)||0, finalHp=Math.max(1,Number(hp)||1);
  if(color){
  ...
  const unit=_makePanelSummonUnit({name:fullName,atk:finalAtk,hp:finalHp,color,race:'召喚'},[]);
...
function _canPayPanelMana(panel){
  ...
  return _ensureMana()>=Number(panel.manaCost||0);
}
function _payPanelMana(panel){
  const cost=Number(panel.manaCost||0);
  if(_ensureMana()<cost) return false;
  G.mana-=cost;
  if(typeof renderManaHud==='function') renderManaHud();
  _recomputeDynamicPanelStats();
  return true;
...
function _afterPanelSummon(unit,isEnemySide){
  ...
  if(wild) _gainMana(wild,unit.name);
}
...
// ── 「Xマナ：効果」形式の説明文を持つキャラクター・強化パネル共通のマナ発動効果 ──
// マナがXに達した時点で即座に1戦闘1回だけ発動する（スペルのmanaCost仕組みを流用）
...
function _checkManaThresholdUnitEffects(){
  ...
  try{
    const visit=(unit,isEnemySide)=>{
      if(!unit||unit.hp<=0||unit._manaThresholdFired||!unit.manaCost) return;
      if(!_canPayPanelMana(unit)) return;
      if(!_payPanelMana(unit)) return;
      unit._manaThresholdFired=true;
      const m=String(unit.desc||'').match(/^\d+マナ：\s*(.+)/);
...
function applyNewPanelBattleStart(){
  ...
  (G.allies||[]).forEach(owner=>{
    ...
    const orderedPanels=battleOrder||(Array.isArray(owner.equipment)?owner.equipment.filter(p=>p):[]);
  ...
  compactBattleUnits();
  // タイタン：開戦：全ての敵に弱体3を与える。
  if((G.allies||[]).some(a=>a&&a.hp>0&&a.name==='タイタン')){
    (G.enemies||[]).forEach(e=>{
      if(e&&e.hp>0&&!(e.keywords||[]).includes('弱体化3')){
        e.keywords=[...(e.keywords||[]),'弱体化3'];
      }
    });
    log('タイタンの効果で全ての敵は弱体化3を得た。','good');
  }
  // スケルトンキング：開戦：全ての味方は「死亡：「青スケルトン」（3/1）を召喚する。」を得る。
  if((G.allies||[]).some(a=>a&&a.hp>0&&a.name==='スケルトンキング')){
    (G.allies||[]).forEach(a=>{
      if(a&&a.hp>0) a._grantedDeathSummon={name:'青スケルトン',atk:3,hp:1};
    });
    log('スケルトンキングの効果で全ての味方は死亡時に「青スケルトン」を召喚するようになった。','good');
  }
  _recomputeDynamicPanelStats();
}
...
async function _applyDeathKeywordEffects(unit, unitIsEnemy){
  ...
  if(unit.goldOnDeath){
    onGoldGained(unit.goldOnDeath);
    log(`${_lc(unit.name,unitIsEnemy)}の効果で${unit.goldOnDeath}ゴールドを得た。`,unitIsEnemy?'bad':'good');
  }
  // レイス：死亡：「レイス」以外のランダムな味方に復活を付与する。
  if(unit.name==='レイス'){
    const candidates=allies.filter(a=>a&&a.hp>0&&a!==unit&&a.name!=='レイス'&&!_unitHasKeyword(a,'復活'));
    if(candidates.length){
      const target=candidates[Math.floor(Math.random()*candidates.length)];
      target.keywords=[...(target.keywords||[]),'復活'];
      log(`${_lc(unit.name,unitIsEnemy)}の効果で${_lc(target.name,unitIsEnemy)}に復活を付与した。`,unitIsEnemy?'bad':'good');
    }
  }
...
async function allyAttackAction(ally, allyIdx){
  ...
  if(ally.hp>0&&ally.manaOnAttack) _gainMana(ally.manaOnAttack,ally.name);
...
async function processAllyDeath(unit){
  ...
  // レムレース：死亡：このキャラクターを死亡させたキャラクターに変身する。
  if(unit.name==='レムレース'&&unit._lastDamageSource&&unit._lastDamageSource!==unit&&unit._lastDamageSource.hp>0){
    const killer=unit._lastDamageSource;
    log(`${_lc(unit.name,false)}が死亡し、${_lc(killer.name,(G.enemies||[]).includes(killer))}に変身した。`,'good');
    unit.name=killer.name;
    unit.race=killer.race||unit.race;
    unit.atk=Math.max(0,killer.atk||0);
    unit.baseAtk=Math.max(0,killer.baseAtk||killer.atk||0);
    unit.maxHp=Math.max(1,killer.maxHp||killer.hp||1);
    unit.hp=unit.maxHp;
    unit.color=killer.color||unit.color;
    unit.keywords=[...(killer.keywords||[])];
    unit.desc=killer.desc||'';
...
function _onAnyCharDeath(deadUnit){
  ...
  // リッチ：常時：味方が死亡するたび、ランダムな敵に4ダメージを与える。
  if(!deadIsEnemy){
    (G.allies||[]).filter(u=>u&&u.hp>0&&u.name==='リッチ').forEach(lich=>{
      const alive=(G.enemies||[]).filter(e=>e&&e.hp>0);
      if(!alive.length) return;
      const target=alive[Math.floor(Math.random()*alive.length)];
      log(`${_lc(lich.name,false)}の効果で${_lc(target.name,true)}に4ダメージを与えた。`,'good');
      dealDmgToEnemy(target,4,G.enemies.indexOf(target),lich);
    });
  }
}
