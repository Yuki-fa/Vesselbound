...
const REWARD_GRID_CAPACITY=6; // 報酬置き場：横2×縦3マス固定
...
function _pushToRewardArea(card){
  if(!card) return true;
  if(!Array.isArray(_rewCards)) _rewCards=[];
  const emptyIdx=_findEmptyRewardSlot();
  if(emptyIdx<0) return false;
  return _pushToRewardAreaAt(card,emptyIdx);
}
...
function _pushToRewardAreaAt(card,idx){
  ...
  delete moved._isOriginalReward;
  _rewCards[idx]=moved;
  return true;
}
...
function placePendingPanelToSelectedUnit(slotIdx){
  ...
  if(oldCard&&!merged&&!_pushToRewardArea(oldCard)) return false;
...
function goToReward(){
  ...
  // メイン置き場は最初から全枠使用可能になったため、ボス撃破によるパネル枠解放は不要になった
...
function _dragSrcCard(){
  ...
  if(_dragSrc.arr==='unitEquip') return (G.allies[_dragSrc.unitIdx]?.equipment||[])[_dragSrc.idx]||null;
  ...
  if(card._rewardReturnCard){
    if(_restoreRewardReturnCard(card)){
      if(src.arr==='unitEquip'){
        const unit=G.allies[src.unitIdx];
        if(unit&&unit.equipment) unit.equipment[src.idx]=null;
      } else if(src.arr==='spellSlots'){
        G.spellSlots[src.idx]=null;
      }
    } else return;
  } else {
    const placed=(Number.isInteger(targetIdx)&&targetIdx>=0)?_pushToRewardAreaAt(card,targetIdx):_pushToRewardArea(card);
    if(!placed) return;
    if(src.arr==='unitEquip'){
      const unit=G.allies[src.unitIdx];
      const nextEquips=(unit?.equipment||[]).slice();
      nextEquips[src.idx]=null;
      if(unit&&!_canApplyUnitEquipChange(unit,nextEquips)) return;
      _clearStarterPanelMarker(unit,src.idx,card);
      if(unit) unit.equipment[src.idx]=null;
      if(unit){ _syncUnitPanelEffectsAfterMove(unit); if(typeof syncEquipmentPassives==='function') syncEquipmentPassives(); }
    } else if(src.arr==='spellSlots'){
      G.spellSlots[src.idx]=null;
    }
  }
...
function renderRewCards(){
  ...
  if(!el._wiredForReturn){
    ...
    el.addEventListener('dragover',e=>{
      if(_dragSrc&&(_dragSrc.arr==='unitEquip'||_dragSrc.arr==='spellSlots')){ e.preventDefault(); el.classList.add('drag-over'); }
    });
    el.addEventListener('dragleave',()=>el.classList.remove('drag-over'));
    el.addEventListener('drop',e=>{
      e.preventDefault(); el.classList.remove('drag-over');
      if(!_dragSrc||(_dragSrc.arr!=='unitEquip'&&_dragSrc.arr!=='spellSlots')) return;
  ...
  const _rewSection=document.getElementById('reward-cards-section');
  if(G._isShop){
    if(_rewSection) _rewSection.classList.remove('reward-grid-mode');
  ...
  // 報酬置き場：スペル置き場と同様の横2×縦3マス固定グリッド。左上・右上・左中・右中の4枠に提示カードを配置し、
  // 下段2枠は空欄（card_back_o.png）として、装備欄等から戻したカードの置き場になる。
  if(_rewSection) _rewSection.classList.add('reward-grid-mode');
  _rewCards=_rewCards.filter(card=>!card||!card._isChar);
  const rewardGrid=document.createElement('div');
  rewardGrid.className='reward-grid-zone';
  for(let i=0;i<REWARD_GRID_CAPACITY;i++){
    ...
    if(card){
      ...
      const ph=document.createElement('div');
      ph.className='card-empty spell reward-grid-empty';
      ph.addEventListener('dragover',e=>{
        if(_dragSrc&&(_dragSrc.arr==='unitEquip'||_dragSrc.arr==='spellSlots')){ e.preventDefault(); e.stopPropagation(); ph.classList.add('drag-over'); }
      });
      ph.addEventListener('dragleave',()=>ph.classList.remove('drag-over'));
      ph.addEventListener('drop',e=>{
        ...
        _returnDragSrcToRewardArea(i);
      });
      rewardGrid.appendChild(ph);
    }
  }
  el.appendChild(rewardGrid);
...
// 戦闘順序：報酬カードが元々あった位置に、魔術師（初期キャラ）以外のメイン置き場キャラを並べ替え可能な状態で表示する。
// メイン置き場（装備欄）内の物理的な位置とは独立した並び順（unit._battleOrder、パネル参照の配列）で管理し、
// どちらか一方を並べ替えてももう一方の位置には影響しない。この並び順が戦闘開始時のパネル召喚順に反映される。
let _battleOrderDragSrc=-1;
function renderBattleOrderRow(show){
  ...
  const order=unit&&typeof _syncUnitBattleOrder==='function'?_syncUnitBattleOrder(unit):null;
  if(!show||G.phase!=='reward'||!order){ section.style.display='none'; row.innerHTML=''; return; }
  section.style.display='';
  row.innerHTML='';
  order.forEach((panel,pos)=>{
    const idx=(unit.equipment||[]).indexOf(panel);
    if(idx<0) return;
    const div=document.createElement('div');
    div.className='card panel character-card panel-character-card';
    const _boShield=String([panel?.desc,panel?.effectText,panel?.effect,panel?.name].filter(Boolean).join(' ')).includes('シールド');
    if(_boShield) div.classList.add('shield-active');
    if(Number(unit.hp||0)<=0){
      div.classList.add('unit-dead','disabled');
      div.style.opacity='0.45';
      div.style.filter='grayscale(0.85) brightness(0.55)';
    }
    if(panel.rarity>=1&&panel.rarity<=5) div.classList.add(`rarity-${panel.rarity}`);
    div.dataset.equipIdx=String(idx);
    div.dataset.battlePos=String(pos);
    if(typeof applyCardVisual==='function') applyCardVisual(div,panel);
    else if(typeof getCardAsset==='function'&&typeof assetUrl==='function') div.style.setProperty('--card-art',assetUrl(getCardAsset(panel)));
    const st=typeof _panelCharacterPreviewStats==='function'?_panelCharacterPreviewStats(unit,idx,panel):{atk:panel.power??0,hp:panel.life??1};
    const panelEffects=typeof _enchantmentEffectsList==='function'?_enchantmentEffectsList(unit,idx):[];
    const preview=[panel.name,typeof _previewRarityLine==='function'?_previewRarityLine(panel):'',panel.desc||'',panelEffects.map(e=>e.text).join('\n')].filter(Boolean).join('\n');
    if(preview) div.setAttribute('data-preview',preview);
    div.innerHTML=`<div class="card-art"></div><span class="card-summon-atk">${st.atk}</span><span class="card-summon-hp">${st.hp}</span>`;
    if(typeof _wireEnchantGlowHover==='function') _wireEnchantGlowHover(div,unit,G._selectedEquipUnitIdx,idx);
    // メイン置き場の同じキャラクターパネルにも同様の発光を同期させる
    div.addEventListener('mouseenter',()=>{
      const handSlots=document.getElementById('hand-slots');
      const el=handSlots&&typeof _equipSlotEl==='function'?_equipSlotEl(handSlots,idx):null;
      if(el) el.classList.add('glow-blue');
    });
    div.addEventListener('mouseleave',()=>{
      const handSlots=document.getElementById('hand-slots');
      const el=handSlots&&typeof _equipSlotEl==='function'?_equipSlotEl(handSlots,idx):null;
      if(el) el.classList.remove('glow-blue');
    });
    div.draggable=true;
    div.addEventListener('dragstart',e=>{
      _battleOrderDragSrc=pos;
      e.dataTransfer.effectAllowed='move';
      e.dataTransfer.setDragImage(_transparentDragImg,0,0);
      _setDragZoneClass('dragzone-battleorder');
      _createDragGhost(div);
      div.classList.add('dragging');
    });
    div.addEventListener('drag',e=>{ if(e.clientX||e.clientY) _moveDragGhost(e.clientX,e.clientY); });
    div.addEventListener('dragend',()=>{ div.classList.remove('dragging'); _removeDragGhost(); _battleOrderDragSrc=-1; });
    div.addEventListener('dragover',e=>{
      if(_battleOrderDragSrc<0||_battleOrderDragSrc===pos) return;
      e.preventDefault();
    ...
    div.addEventListener('drop',e=>{
      ...
      const srcPos=_battleOrderDragSrc; _battleOrderDragSrc=-1;
      if(srcPos<0||srcPos===pos) return;
      // 装備欄配列とは独立した「戦闘順序」配列内でのみ入れ替える（メイン置き場の物理位置には触れない）
      const liveUnit=G.allies?.[G._selectedEquipUnitIdx];
      const liveOrder=liveUnit&&typeof _syncUnitBattleOrder==='function'?_syncUnitBattleOrder(liveUnit):order;
      const tmp=liveOrder[srcPos]; liveOrder[srcPos]=liveOrder[pos]; liveOrder[pos]=tmp;
      renderBattleOrderRow(true);
      renderHandEditor();
    });
    row.appendChild(div);
  });
...
function _renderFieldRow(el){
  ...
  for(let i=0;i<maxAllies;i++){
    ...
    if(unit){
      ...
      const raceTag='';
      const _kColorMap={'即死':'#e060e0','侵食':'#a060d0','加護':'#60b0e0','エリート':'#ffd700','ボス':'#ff8040','二段攻撃':'#60d0e0','三段攻撃':'#60d0e0','全体攻撃':'#e04040','狩人':'#d08040','結束':'#80d0d0','邪眼':'#c060c0','シールド':'#60a0e0','標的':'#60c0c0','成長':'#60d090'};
      ...
      const _allKws=[...new Set(unit.keywords||[])];
...
// メイン置き場：7列×3行＝21枠。リーダー専用枠は存在せず、全枠が最初から使用可能。
// キャラクターパネルは■(character)の枠、強化パネルは□(enchant)の枠にしか置けない。
//   □■□■□■□
//   □□□□□□□
//   ■□■□■□■
const _EQUIP_SLOT_KIND_PATTERN=[
  'enchant','character','enchant','character','enchant','character','enchant',
  'enchant','enchant','enchant','enchant','enchant','enchant','enchant',
  'character','enchant','character','enchant','character','enchant','character',
];
const UNIT_EQUIP_SLOTS=_EQUIP_SLOT_KIND_PATTERN.map(kind=>({label:'',kind}));
function _ensureUnitPanelBoard(unit){
  if(!unit) return;
  const eq=unit.equipment||[];
  if(!unit._starterWolfSeeded&&typeof makePanel==='function'){
    const avail=[];
    for(let i=0;i<UNIT_EQUIP_SLOTS.length;i++) if(UNIT_EQUIP_SLOTS[i].kind==='character'&&!eq[i]) avail.push(i);
...
function _canCardUseEquipSlot(card,idx,unit){
  ...
  const kind=_equipSlotDef(idx,unit).kind;
  const cat=String(card.category||'');
  if(kind==='character') return cat==='キャラクター';
  if(kind==='enchant') return cat==='強化'||cat==='エンチャント';
  return true;
}
...
function _canApplyUnitEquipChange(unit,nextEquips){
  ...
  if(next<cur&&((unit.hp||0)+(next-cur))<=0) return false;
  return true;
}
...
function renderSpellSlotZone(el){
  ...
  for(let idx=0;idx<3;idx++){
    ...
    if(card){
      ...
      if(preview) div.setAttribute('data-preview',preview);
      const orbImgs=div.querySelectorAll('.mana-cost-orbs img');
      if(G.phase==='player'||G.phase==='enemy'){
        // 戦闘中（自動戦闘解決を含む）のみ：発動済みは非表示、未発動はマナが足りるまで暗く・貯まった分だけオーブを明るく
        const have=Number(G.mana)||0;
        const need=Number(card.manaCost||0);
        if(card._firedThisBattle){
          div.classList.add('spell-fired');
        } else {
          div.classList.toggle('spell-dim',have<need);
          orbImgs.forEach((img,i)=>{ if(i<have) img.classList.add('mana-orb-lit'); });
        }
      } else {
        // 非戦闘中（報酬フェイズ等）：スペル・オーブは常に明るいまま
...
function renderHeRow(elId, arr, startIdx, count, arrName){
  ...
  for(let i=startIdx;i<startIdx+Hcols;i++){
    ...
    if(card){
      ...
      const _dirOwner=arrName==='unitEquip'?G.allies?.[G._selectedEquipUnitIdx]:null;
      ...
      if(_isPanelCharacter){
        const _panelOwner=arrName==='unitEquip'?G.allies?.[G._selectedEquipUnitIdx]:null;
        const st=_panelCharacterPreviewStats(_panelOwner,arrName==='unitEquip'?i:null,card);
        const pAtk=st.atk, pHp=st.hp;
        const _panelEffects=_panelOwner&&typeof _enchantmentEffectsList==='function'?_enchantmentEffectsList(_panelOwner,i):[];
        const preview=[card.name,typeof _previewRarityLine==='function'?_previewRarityLine(card):'',card.desc||'',_panelEffects.map(e=>e.text).join('\n')].filter(Boolean).join('\n');
        if(preview) div.setAttribute('data-preview',preview);
        ...
        if(arrName==='unitEquip'&&typeof _wireEnchantSelfHover==='function') _wireEnchantSelfHover(div,G.allies?.[G._selectedEquipUnitIdx],i);
      ...
      ph.addEventListener('drop',e=>{
        ...
        if(arrName==='unitEquip'&&_dragSrc){
          ...
          const srcCard=_dragSrc.arr==='inventory'?G.inventory[_dragSrc.idx]
            :_dragSrc.arr==='unitEquip'?(G.allies[_dragSrc.unitIdx]?.equipment||[])[_dragSrc.idx]
  ...
  if(arrName==='unitEquip'){
    const _uniteOwner=G.allies?.[G._selectedEquipUnitIdx];
...
function _renderPanelUniteMarkers(host, unit){
  ...
  eq.forEach((panel,idx)=>{
    ...
    panel.directions.forEach(d=>{
      ...
      const isCharTarget=targetIdx===0||(targetPanel&&String(targetPanel.category||'')==='キャラクター');
      ...
      const dstEl=targetIdx===0?host.querySelector('.reward-unit-anchor'):host.querySelector(`[data-equip-idx="${targetIdx}"]`);
      if(!srcEl||!dstEl) return;
      const vertical=(d==='up'||d==='down');
      const sx=srcEl.offsetLeft, sy=srcEl.offsetTop, sw=srcEl.offsetWidth, sh=srcEl.offsetHeight;
      const dx=dstEl.offsetLeft, dy=dstEl.offsetTop, dw=dstEl.offsetWidth, dh=dstEl.offsetHeight;
      let midX,midY;
      if(vertical){
...
function dropOnCard(destArr,destIdx){
  ...
  if(destArr==='unitEquip'){
    ...
    if(srcArr==='unitEquip'&&srcUnitIdx!==G._selectedEquipUnitIdx) return;
