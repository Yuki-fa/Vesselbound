...
async function loadGameData() {
  try {
    ...
    const _JA_TO_MANA_COLOR = { '赤': 'red', '青': 'blue', '緑': 'green', '黄': 'yellow', '茶': 'yellow' };
    const _manaColorName = color => _JA_TO_MANA_COLOR[String(color || '').trim()] || '';
    ...
    const _setPanelKeywordsFromDesc = panel => {
      ...
      const kws = [];
      if (/復活/.test(desc)) kws.push('復活');
      if (/シールド/.test(desc)) kws.push('シールド');
      if (/根性/.test(desc)) kws.push('根性');
      const poison = desc.match(/毒牙\s*(\d*)/);
      if (poison) kws.push('毒牙' + (poison[1] || '1'));
      const strengthen = desc.match(/([赤青緑黄茶])強化/);
      if (strengthen) kws.push(`${strengthen[1]}強化`);
      if (/全ての赤キャラクターは\+3\/\+3/.test(desc)) kws.push('赤全体強化3');
      panel.keywords = kws;
      ...
      const attackMana = desc.match(/攻撃：\s*([赤青緑黄茶])マナ/);
      if (attackMana) panel.manaOnAttack = _manaColorName(attackMana[1]);
      const injuryMana = desc.match(/負傷：\s*([赤青緑黄茶])マナ/);
      if (injuryMana) panel.manaOnInjury = _manaColorName(injuryMana[1]);
    };
    const _setEnchantFieldsFromDesc = panel => {
      ...
      const strengthen = desc.match(/([赤青緑黄茶])強化/);
      if (strengthen) panel.adjacentKeywords.push(`${strengthen[1]}強化`);
      const poison = desc.match(/毒牙\s*(\d*)/);
      if (poison) panel.adjacentKeywords.push('毒牙' + (poison[1] || '1'));
      const colorMana = desc.match(/召喚：\s*([赤青緑黄茶])マナ/);
      if (colorMana) panel.adjacentKeywords.push(`${colorMana[1]}マナ召喚`);
    ...
    const _syncPanelFromRow = (panel, row, forcedCategory) => {
      ...
      if (color) {
        panel.color = color;
        const mc = _JA_TO_MANA_COLOR[color];
        if (mc) panel.manaColor = mc;
      }
