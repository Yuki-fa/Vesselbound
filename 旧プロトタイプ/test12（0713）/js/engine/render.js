...
// 説明文中の色名（青・赤・緑・黄・茶）にマナアイコンを添える（エスケープ済みテキストに対して実行する）
function _injectManaIcons(escapedText){
  if(typeof _manaOrbPath!=='function') return escapedText;
  const _descManaColorMap={'青':'blue','赤':'red','緑':'green','黄':'yellow','茶':'yellow'};
  const icon=m=>{
    const path=_manaOrbPath(_descManaColorMap[m]);
    return path?`<img class="desc-mana-icon" src="${path}" alt="${m}">`:m;
  };
  return String(escapedText||'').split(/(<[^>]*>)/g).map(part=>{
    if(part.startsWith('<')) return part;
    return part
      .replace(/([青赤緑黄茶])マナ/g,(_,m)=>`${icon(m)}マナ`)
      .replace(/([青赤緑黄茶])(?=強化|キャラクター|カード|コスト|色)/g,m=>icon(m));
  }).join('');
}
...
function _unitPreviewText(unit, desc, slotIdx){
  ...
  const _isInternalOnlyKeyword=k=>/^[赤青緑黄茶]全体強化\d*$/.test(k);
