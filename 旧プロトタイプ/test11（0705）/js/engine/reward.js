...
function _mkRewDiv(card, onBuy, rewIdx){
  ...
  if(rewIdx!=null){
    const _rewardDragLocked=!!G._pendingPanelPlacement||(_rewFreePickDone&&!card._rewardReturnCard);
...
function _createDragGhost(srcEl){
  ...
  const copyStatStyle=(srcSel,dstSel)=>{
    ...
    const fs=parseFloat(s.fontSize)||Math.max(24,Math.min(W,H)*0.16);
  ...
  copyStatStyle('.slot-stats .a','.slot-stats .a');
  copyStatStyle('.slot-stats .h','.slot-stats .h');
  ...
  for(let idx=0;idx<6;idx++){
    ...
    if(card){
      ...
      if(_isPanelCharacter){
        ...
        div.innerHTML=`${_manaCostEl}<div class="card-art"></div><span class="card-summon-atk">${pAtk}</span><span class="card-summon-hp">${pHp}</span>${card._rewardReturnCard?'<button class="discard-btn reward-return-btn" title="報酬に戻す">×</button>':''}`;
      }else{
        ...
        if(_isEnchantPanel){
          ...
          const preview=[card.name,typeof _previewRarityLine==='function'?_previewRarityLine(card):'',computeDesc(card)||card.desc||''].filter(Boolean).join('\n');
          if(preview) div.setAttribute('data-preview',preview);
          const _manaCostEl=typeof cardManaCostHtml==='function'?cardManaCostHtml(card):'';
          div.innerHTML=`${_manaCostEl}${_dirMarks}<div class="card-art"></div>${card._rewardReturnCard?'<button class="discard-btn reward-return-btn" title="報酬に戻す">×</button>':''}`;
        }else{
...
function renderHeRow(elId, arr, startIdx, count, arrName){
  ...
  for(let i=startIdx;i<startIdx+Hcols;i++){
    ...
    if(card){
      ...
      const _gradeCount=_usesRarityStars?0:(card.grade||1);
      const _gradeEl=_gradeCount>0?`<span class="card-grade${card.legend?' legend-grade':''}">${typeof gradeIconHtml==='function'?gradeIconHtml(_gradeCount):gradeStr(_gradeCount)}</span>`:'';
      const _manaCostEl=typeof cardManaCostHtml==='function'?cardManaCostHtml(card):'';
