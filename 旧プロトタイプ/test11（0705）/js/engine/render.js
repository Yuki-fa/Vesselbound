...
function _formatPreviewHtml(desc){
  ...
  return clean.split('\n').map((line,li)=>{
    const star=line.match(/^__RARITY_STARS__:(\d+)/);
    if(star){
      const n=Math.max(1,Math.min(5,Number(star[1])||1));
      return `<span class="preview-stars">${Array.from({length:n},()=>`<img src="${assetUrl(Assets.cards.gradeStar)}" alt="">`).join('')}</span>`;
    }
...
function _previewRarityLine(card){
  ...
  if((card?.type==='panel'||card?.kind==='panel'||card?.panelScope)&&['キャラクター','強化','エンチャント'].includes(cat)){
    const n=Number(card.rarity);
    if(n>0) return `__RARITY_STARS__:${n}`;
  }
  return '';
}
function _manaOrbPath(color){
...
function cardManaCostHtml(card){
  ...
  const path=n?_manaOrbPath(card?.manaColor||card?.color):'';
  if(!n||!path) return '';
  return `<span class="mana-cost-orbs">${Array.from({length:n},()=>`<img src="${assetUrl(path)}" alt="">`).join('')}</span>`;
}
...
function playAttackMotion(attacker,target,isEnemySide){
  ...
  const copyMotionStat=(srcSel,dstSel)=>{
    const src=fromEl.querySelector(srcSel);
    const dst=clone.querySelector(dstSel);
    if(!src||!dst) return;
    const s=getComputedStyle(src);
    const sr=src.getBoundingClientRect();
    const rr=fromEl.getBoundingClientRect();
    const fs=parseFloat(s.fontSize)||Math.max(24,Math.min(fr.width,fr.height)*0.16);
    dst.style.setProperty('font-size',`${fs}px`,'important');
    dst.style.setProperty('line-height',s.lineHeight&&s.lineHeight!=='normal'?s.lineHeight:'1','important');
  ...
  if(stats){
    ...
    copyMotionStat('.slot-stats .a','.slot-stats .a');
    copyMotionStat('.slot-stats .h','.slot-stats .h');
  }
...
function renderManaHud(){
  ...
  ].map(([k,label])=>`<div class="mana-row mana-${k}"><span>${label}</span><b>${Math.max(0,Number(m[k])||0)}</b></div>`).join('');
}
...
function mkCardEl(card,_idx,_ctx,_mlOverride){
  ...
  const _gradeNum=_usesRarityStars?0:(card.grade||(card._rarity)||((card.rarity>0)?card.rarity:null)||((card.type==='wand'||card.type==='consumable')?1:0));
  ...
  if(isPanelCard&&['強化','エンチャント'].includes(String(card.category||''))){
    ...
    const preview=[card.name,_previewRarityLine(card),dynDesc||card.desc||''].filter(Boolean).join('\n');
    if(preview) div.setAttribute('data-preview',preview);
