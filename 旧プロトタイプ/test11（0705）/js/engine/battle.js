...
function _manaKey(color){
  ...
  if(c==='黄'||c==='茶'||c==='yellow'||c==='brown') return 'yellow';
  return '';
}
function _manaLabel(key){
  return {red:'赤',blue:'青',green:'緑',yellow:'黄'}[key]||key;
}
...
function _gainMana(color, amount, source){
  ...
  m[key]=(m[key]||0)+n;
  log(`${source||'マナ'}：${_manaLabel(key)}マナ+${n}`,'good');
  if(typeof renderManaHud==='function') renderManaHud();
  _checkManaCostSummons();
}
...
function _afterPanelSummon(unit,isEnemySide){
  ...
  if(_unitHasKeyword(unit,'野生の力')) _gainMana('green',1,unit.name);
}
function _checkManaCostSummons(){
  if(G._checkingManaSummons) return;
  G._checkingManaSummons=true;
  try{
    (G.allies||[]).forEach(owner=>{
      if(!owner||owner.hp<=0) return;
      _forEachUnitPanel(owner,(panel,idx)=>{
        if(!panel||String(panel.category||'')!=='キャラクター'||String(panel.subCategory||'')!=='特殊') return;
        if(panel._specialSummoned||!panel.manaCost||!_canPayPanelMana(panel)) return;
        if(!_payPanelMana(panel)) return;
        _summonSpecialCharacterPanel(panel.name,`${panel.name}：${_manaLabel(_manaKey(panel.manaColor||panel.color||panel.カラー))}マナで召喚`);
      });
    });
  }finally{
    G._checkingManaSummons=false;
  }
}
    ...
    _forEachUnitPanel(owner,(panel,idx)=>{
      ...
      for(let n=0;n<(spec.count||1);n++){
        ...
        if(_summonPanelUnitToFront(summoned,false)>=0){
          ...
          log(`${panel.name}：${summoned.name}を召喚`,'good');
        }
      }
    });
  });
...
function _summonSpecialCharacterPanel(name, logText){
  ...
  if(name==='アークデーモン'){
    const front=(G.enemies||[]).filter(e=>e&&e.hp>0&&!e._isObject&&!e._isSoul&&(e.lane||'front')==='front');
    const t=front.length?randFrom(front):null;
    if(t){
      t.hp=0;
      log(`アークデーモン：${t.name}を破壊`,'bad');
      processEnemyDeath(t,G.enemies.indexOf(t));
    }
  }
  if(name==='アラッサス'){
...
function _summonMataOnAllyInjury(injured){
  ...
  sources.forEach(src=>{
    ...
    const unit=_makePanelSummonUnit({
      name:'マータ',
      atk:Number(src.panel.power??src.panel.atk??0),
      hp:Number(src.panel.life??src.panel.hp??7),
      race:src.panel.race||'',
      desc:src.panel.desc||'',
      keywords:src.panel.keywords||['生贄'],
      color:src.panel.color||src.panel.カラー||'赤',
      manaOnInjury:src.panel.manaOnInjury||'red',
      panelName:'マータ'
    },enh.keywords||[]);
    _applyAdjacentPanelEnhancements(unit,enh);
    if(_summonPanelUnitToFront(unit,false)>=0){
      _afterPanelSummon(unit,false);
      log('マータ：味方の負傷に応じて召喚','good');
    }
  });
}
