...
function doSmithy(regen=true){
  ...
  // 選択肢1：次の報酬にユニークが1つ含まれる
  const doneUnique=_smithyChosen.has('unique');
  const o1=document.createElement('div');
  o1.className='choice-opt'+(doneUnique?' done':'');
  o1.innerHTML='<div class="choice-icon">✨</div><div class="choice-label">神の寵愛</div><div class="choice-desc">次の報酬にユニークが1つだけ含まれる（キャラクター、杖、アイテムの何れかが同種のユニークで上書きされる）</div>';
  if(!doneUnique) o1.onclick=()=>{
    G._nextRewardUniqueSlot=true; updateHUD();
    if(hasFarsight){ log('次の報酬にユニーク1枚を含む','good'); _smithyChosen.add('unique'); doSmithy(false); }
    else showEvent('祭壇','神の寵愛を受けた。','次の報酬にユニークが1つ含まれる');
  };
  el.appendChild(o1);
  ...
  if(!doneIncome) o2.onclick=()=>{
    ...
    else showEvent('祭壇','豊穣の祝福を受けた。',`基礎収入+1（現在${G.baseIncome}）`);
  };
  el.appendChild(o2);

  // 選択肢3：全仲間±0/+7×現在のグレード
  const doneHeal=_smithyChosen.has('heal');
  const healAmt=7*(G.rewardGrade||1);
  const o3=document.createElement('div');
  o3.className='choice-opt'+(doneHeal?' done':'');
  o3.innerHTML=`<div class="choice-icon">💚</div><div class="choice-label">祭壇の癒し</div><div class="choice-desc">全ての仲間のライフが+${healAmt}される（±0/+7×グレード${G.rewardGrade||1}）</div>`;
  if(!doneHeal) o3.onclick=()=>{
    const amt=7*(G.rewardGrade||1);
    G.allies.forEach(a=>{ if(a&&a.hp>0){ a.hp+=amt; a.maxHp+=amt; }}); updateHUD();
    if(hasFarsight){ log(`全仲間ライフ+${amt}`,'good'); _smithyChosen.add('heal'); doSmithy(false); }
    else showEvent('祭壇','祭壇の祝福を受けた。',`全仲間ライフ+${amt}`);
  };
  el.appendChild(o3);
