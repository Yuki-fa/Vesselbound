...
async function startBattle(){
  ...
  // 敵は前衛5体・後衛3体の最大8枠へ整列。オブジェクトは出現させない。
  {
    ...
    _layoutEnemyLanes(_actualEnemies);
...
function _layoutEnemyLanes(enemies){
  ...
  front=front.slice(0,ENEMY_FRONT_SLOTS||5);
  rear=rear.slice(0,ENEMY_REAR_SLOTS||3);
...
function compactBattleUnits(){
  ...
  (G.allies||[]).slice(0,maxA).forEach((a,i)=>{
    if(a&&a.hp>0&&!a._isSoul&&!a._isObject){
      a.lane=i>=5?'rear':'front';
      nextAllies[i]=a;
    }
  });
  G.allies=nextAllies;
  ...
  (G.enemies||[]).slice(0,maxE).forEach((e,i)=>{
    if(e&&e.hp>0&&!e._isObject){
      e.lane=i>=5?'rear':'front';
      nextEnemies[i]=e;
    }
  });
...
function handleBattleDefeat(){
  ...
  G.life=Math.max(0,(G.life==null?3:G.life)-1);
  if(G.life<=0){ gameOver(); return; }
  restoreAlliesForRewardTransition();
  clearLog();
  log(`敗北：ライフ-1。直前の報酬フェイズに戻ります。`,'bad');
  G.phase='reward';
  G._selectedBattleMagic=null;
  G._battleMagicUsed=false;
  updateHUD();
...
function _dealAttackDamageWithMutual(attacker,isEnemySide,target,targetIdx,damage){
  if(!attacker||!target) return null;
  const actualTarget=_dealAttackDamage(attacker,isEnemySide,target,targetIdx,damage);
  const defender=actualTarget||target;
  const backDmg=Math.max(0,defender?.atk||0);
  if(!isEnemySide&&defender&&(defender.lane||'front')==='rear') return actualTarget;
  if(backDmg>0&&attacker.hp>0){
    ...
    log(`${defender.name}(${backDmg})→${attacker.name}`,isEnemySide?'good':'bad');
  }
  return actualTarget;
}
...
function getAttackTarget(attacker, targets){
  ...
  const isFront=u=>(u.hate&&u.hateTurns>0)||u.lane==='front'||_unitHasKeyword(u,'守護');
  ...
  const frontLine=live.filter(u=>isFront(u)&&!isStealth(u));
  const pool=frontLine.length>0?frontLine:live.filter(u=>!isStealth(u));
...
async function allyAttackAction(ally, allyIdx){
  ...
  const allyField=document.getElementById('f-ally');
  const enemyField=document.getElementById('f-enemy');
  const aSlot=allyField?.querySelector(`.slot[data-unit-idx="${allyIdx}"]`);
  const eSlot=enemyField?.querySelector(`.slot[data-unit-idx="${eIdx}"]`);
  ...
  if(isGlobal){
    const allTgtSlots=liveE.map(e=>enemyField?.querySelector(`.slot[data-unit-idx="${G.enemies.indexOf(e)}"]`)).filter(Boolean);
    ...
    showAttackLine(aSlot,_tdIdxs.map(i=>enemyField?.querySelector(`.slot[data-unit-idx="${i}"]`)).filter(Boolean),'#4080ff');
  } else {
  ...
  log(`${ally.name}(${attackDmg})→${_atkLabel}`);

  attackTargets.forEach(t=>{
    const ti=G.enemies.indexOf(t);
    _dealAttackDamageWithMutual(ally,false,t,ti,attackDmg);
  ...
  if(ally.hp>0&&!isGlobal&&!isTriDir){
    ...
    for(let hi=0;hi<extraHits;hi++){
      ...
      // 各段ごとにアニメーション
      const hitSlot=enemyField?.querySelector(`.slot[data-unit-idx="${G.enemies.indexOf(curTgt)}"]`);
      if(aSlot) aSlot.classList.add('glow-blue');
      ...
      _dealAttackDamageWithMutual(ally,false,curTgt,G.enemies.indexOf(curTgt),attackDmg);
...
async function enemyAttackAction(enemy, enemyIdx){
  ...
  const enemyField=document.getElementById('f-enemy');
  const allyField=document.getElementById('f-ally');
  const eSlot=enemyField?.querySelector(`.slot[data-unit-idx="${enemyIdx}"]`);
  const aSlot=allyField?.querySelector(`.slot[data-unit-idx="${primaryIdx}"]`);
  ...
  if(isGlobalAtk){
    const allTgtSlots=liveAllForGlobal.map(a=>allyField?.querySelector(`.slot[data-unit-idx="${G.allies.indexOf(a)}"]`)).filter(Boolean);
    ...
    showAttackLine(eSlot,_tdAIdxs.map(i=>allyField?.querySelector(`.slot[data-unit-idx="${i}"]`)).filter(Boolean),'#ff4040');
  } else {
  ...
  finalTargets.forEach(tgt=>{
    ...
    if(!hitSet.has(tgt.id)){
      const actualTarget=_dealAttackDamageWithMutual(enemy,true,tgt,aIdx,atkVal);
  ...
  if(!isGlobalAtk&&!isTriDirAtk&&enemy.hp>0){
    ...
    for(let hi=0;hi<extraHits;hi++){
      ...
      // 各段ごとにアニメーション
      const reHitSlot=allyField?.querySelector(`.slot[data-unit-idx="${G.allies.indexOf(reTgt)}"]`);
      if(eSlot) eSlot.classList.add('glow-blue');
      ...
      const actualTarget=_dealAttackDamageWithMutual(enemy,true,reTgt,G.allies.indexOf(reTgt),atkVal);
...
function processAllyDeath(unit){
  ...
  if(_unitHasKeyword(unit,'再生')&&!unit._starterRegenUsed){
    unit._starterRegenUsed=true;
    unit.hp=Math.max(1,unit.maxHp||1);
    log(`${unit.name}：再生→復活`,'good');
    renderAll();
    return;
  }
