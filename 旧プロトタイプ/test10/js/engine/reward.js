...
function carryCap(){
  return 5+Math.max(0,((G.facilities&&G.facilities.vault)||1)-1)*5;
}

function showCarryGoldWarning(onOk){
  const cap=carryCap();
  const old=document.getElementById('carry-gold-warning');
  if(old) old.remove();
  const ov=document.createElement('div');
  ov.id='carry-gold-warning';
  ov.innerHTML=`<div class="carry-warn-box"><div class="carry-warn-title">ゴールド上限超過</div><div class="carry-warn-text">現在 ${G.gold}｜${cap}<br>超過分は失われます。このまま進みますか？</div><div class="carry-warn-actions"><button class="btn" data-act="cancel">キャンセル</button><button class="btn" data-act="ok">進む</button></div></div>`;
  ov.addEventListener('click',e=>{
    const b=e.target.closest('button');
    if(!b) return;
    if(b.dataset.act==='ok'){
      G.gold=Math.min(G.gold,cap);
      updateHUD();
...
function placePendingPanelToSelectedUnit(slotIdx){
  ...
  if(equips[slotIdx]&&equips[slotIdx].fixedEquip){
    log('初期パネルは上書きできません','bad');
    return false;
  }
  const merged=_mergedPanelCard(equips[slotIdx],pending.card);
  equips[slotIdx]=merged||clone(pending.card);
...
function goToReward(){
  if(typeof restoreAlliesForRewardTransition==='function') restoreAlliesForRewardTransition();
  ...
  if(goldLabel) goldLabel.textContent='所持金｜繰越可能金';
...
function chooseMoveInline(nt){
  ...
  if(G.gold>carryCap()){
    showCarryGoldWarning(()=>chooseMoveInline(nt));
    return;
  }
  squirrelSay('退店時');
...
function renderRewCards(){
  ...
  const rbLegacy=document.getElementById('rw-reroll'); if(rbLegacy){ const _rbDis=!!G._pendingPanelPlacement||G.gold<1||(!G._isRewardTown&&_rewFreePickDone); rbLegacy.disabled=_rbDis; rbLegacy.style.opacity=_rbDis?'0.4':''; }
  requestAnimationFrame(fitCardDescs);
  return;
  ...
  const rb=document.getElementById('rw-reroll'); if(rb){ const _rbDis=!!G._pendingPanelPlacement||G.gold<1||(!G._isRewardTown&&_rewFreePickDone); rb.disabled=_rbDis; rb.style.opacity=_rbDis?'0.4':''; }
  requestAnimationFrame(fitCardDescs);
}
...
function takeRewCard(i, targetSlot){
  ...
  if(card.type==='panel'||card.type==='global-panel'||card.kind==='panel'||card.panelScope){
    ...
    if(targetSlot!=null&&typeof placePendingPanelToSelectedUnit==='function'){
      placePendingPanelToSelectedUnit(targetSlot);
    }
    return;
  }
...
function _renderFieldRow(el){
  ...
  for(let i=0;i<maxAllies;i++){
    ...
    const lane=(i>=5?'rear':'front');
    div.style.gridRow=lane==='rear'?'1':'2';
    div.style.gridColumn=String((i%5)+1);
    if(unit){
      const hasGuard=(typeof _unitHasKeyword==='function'&&_unitHasKeyword(unit,'守護'))||((unit.equipment||[]).some(p=>p&&(p.name==='守護'||(p.keywords||[]).includes('守護'))));
      div.className='slot unit-card'+(unit.hp<=0?' dead-unit inert':'')+(unit.hp>0&&((unit.hate&&unit.hateTurns>0)||hasGuard)?' is-defender uses-hate-frame':'')+(G._selectedEquipUnitIdx===i?' selected':'');
...
function sellFieldUnit(idx){
  ...
  (unit.equipment||[]).filter(eq=>eq&&!eq.fixedEquip).forEach(eq=>{
...
const UNIT_EQUIP_SLOTS=[
  {label:'パネル1', kind:'panel'},
  {label:'パネル2', kind:'panel'},
  {label:'パネル3', kind:'panel'},
  {label:'パネル4', kind:'panel'},
  {label:'パネル5', kind:'panel'},
  {label:'パネル6', kind:'panel'},
];
function _normalizeUnitEquipment(unit){
  ...
  if(!next[0]&&unit.initialPanelName&&typeof makeStarterInitialPanel==='function'){
    next[0]=makeStarterInitialPanel(unit.initialPanelName,unit.initialPanelDesc);
  }
  unit.equipment=next;
  syncUnitPanelStatBonuses(unit);
...
function moveEquippedCardToUnit(equipIdx, srcUnitIdx, destUnitIdx){
  ...
  syncUnitPanelStatBonuses(srcUnit);
  syncUnitPanelStatBonuses(destUnit);
...
function unequipFromSelectedUnit(equipIdx){
  ...
  if(_equipSlotDef(equipIdx,unit).fixed||unit.equipment[equipIdx].fixedEquip){ log('初期パネルは外せません','bad'); return; }
  ...
  if(G.phase==='reward'){
    ...
    if(typeof syncEquipmentPassives==='function') syncEquipmentPassives();
    log(`${unit.name} の ${card.name} を破棄`,'sys');
    renderHandEditor();
  ...
  if(handIdx<0){ unit.equipment[equipIdx]=card; syncUnitPanelStatBonuses(unit); log('手札が満杯で外せません','bad'); return; }
...
function renderHeRow(elId, arr, startIdx, count, arrName){
  ...
  el.style.setProperty('grid-template-columns',battleMagic?'repeat(2,var(--hand-card-w))':`repeat(${Hcols},var(--hand-card-w,300px))`,'important');
  ...
  if(elId==='hand-slots'){
    ...
    if(handPane){
      const rewardUnitPanels=G.phase==='reward'&&arrName==='unitEquip';
      ...
      handPane.style.setProperty('width',battleMagic?'var(--right-stack-w)':'2440px','important');
      handPane.style.setProperty('height',battleMagic?'calc(var(--hand-card-h) * 3 + var(--field-gap) * 2)':'var(--hand-card-h)','important');
  ...
  for(let i=startIdx;i<startIdx+Hcols;i++){
    ...
    if(card){
      ...
      if(arrName==='unitEquip'&&G._selectedEquipCardIdx===i) div.classList.add('selected');
      ...
      div.draggable=arrName!=='globalPanels'&&!(arrName==='unitEquip'&&card.fixedEquip);
      ...
      const _spellBtn=arrName==='unitEquip'
        ?(card.fixedEquip?'':`<button class="discard-btn" title="解除">×</button>`)
      ...
      div.innerHTML=`${_slotLabel}${_gradeEl}<div class="card-art"></div><div class="card-tp ${t}">${arrName==='globalPanels'?'全体':arrName==='unitEquip'?'パネル':t==='ring'?'指輪':t==='wand'?'杖':'アイテム'}</div><div class="card-name">${card.name}</div><div class="card-desc">${computeDesc(card)}</div>${_chargeHtml}${_spellBtn}`;
      const discardBtn=div.querySelector('.discard-btn');
      if(discardBtn) discardBtn.onclick=ev=>{
        ev.stopPropagation();
        if(arrName==='unitEquip'){ if(!card.fixedEquip) unequipFromSelectedUnit(i); return; }
...
function dropOnCard(destArr,destIdx){
  ...
  if(destArr==='unitEquip'){
    ...
    if(merged){
      ...
      syncUnitPanelStatBonuses(destUnit);
      if(srcUnit) syncUnitPanelStatBonuses(srcUnit);
      if(typeof syncUnitPanelFlags==='function'){
        syncUnitPanelFlags(destUnit);
        if(srcUnit) syncUnitPanelFlags(srcUnit);
      }
      if(typeof syncEquipmentPassives==='function') syncEquipmentPassives();
