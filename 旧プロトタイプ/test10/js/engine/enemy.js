...
function _sideBossDef(def, grade){
  const same=ENEMY_POOL.find(e=>e!==def && !e.bossOnly && e.grade===grade && e.name===def.name);
  if(same) return same;
  return {
    ...def,
    bossOnly:false,
    keywords:(def.keywords||[]).filter(k=>k!=='ボス'),
  };
}
...
function generateEnemies(floor){
  ...
  if(isBoss){
    ...
    const enemies=[make(sideDef,false),make(bossDef,true),make(sideDef,false)];
    G._bossSlot=1;
    return enemies;
  }
