...
function getCardFrameAsset(card){
  ...
  if(card.type==='panel'||card.kind==='panel'||card.panelScope){
    if(String(card.category||'').includes('戦闘力')) return Assets.cards.itemFrame;
    if(String(card.category||'').includes('成長')) return Assets.cards.growthFrame||Assets.cards.weaponFrame||Assets.cards.itemFrame;
...
function applyUnitVisual(el, unit){
  if(!el) return;
  const hasGuard=!!(unit&&Array.isArray(unit.equipment)&&unit.equipment.some(p=>p&&(p.name==='守護'||(p.keywords||[]).includes('守護'))));
  const isDefender=!!(unit&&(unit.hate&&unit.hateTurns>0||hasGuard))||el.classList.contains('is-defender')||el.classList.contains('uses-hate-frame');
